 ...
 *
 * Whitespace-delimited "words" are meaningless for Japanese and Chinese, and
 * a character-per-word ratio would be an invented number. For those two
 * languages the count is dictionary word segmentation via `Intl.Segmenter`
 * (word granularity, word-like segments only). Where the runtime has no
 * `Intl.Segmenter` — Hermes 0.81.5 as shipped with this app does not — the
 * result is flagged `unavailable` and carries a character count that must
 * never be presented as words. The server (Deno, full ICU) is authoritative.
 *
 * TWIN: supabase/functions/grade-writing/writing-length.ts. The edge function
 * cannot import from lib/ (tsconfig excludes supabase/functions and Deno has
 * no path into the app tree), so the counting logic is duplicated there.
 * Everything below this header must stay byte-identical in the two files;
 * `writing-length.test.ts` on both sides reads both files and fails if it is
 * not, so drift is caught by `npm test` and by `deno test` alike.
 */
