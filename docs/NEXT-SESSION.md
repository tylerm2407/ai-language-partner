...
**Before committing a regenerated seed.sql**, re-run these against the file —
they are the same checks used on production: no old strings, no mojibake (the
PS 5.1 trap in §2.2), no degenerate fill-blank, no multiple-choice missing its
own answer, no duplicate options, no whitespace-padded answers.
