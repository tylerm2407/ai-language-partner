...
| R-008 | Technical | **Single region deployment.** All infrastructure runs in AWS us-east-1. A regional failure (rare but possible) would cause total service unavailability with no geographic failover. | Low | High | 3 | Supabase manages infrastructure redundancy within the region and daily database backups are enabled. PITR, independent object-storage recovery, and a completed restore exercise are outstanding. | Medium — accept only for pre-launch testing. Complete a restore exercise and storage-backup control before institutional commitments; evaluate multi-region when scale or SLAs require it. | Tyler Moore |
