...
const archive=JSON.parse(await readFile(`${base}/reviewed-source-6a50f71788ce.json`,'utf8'));
const decisions=(await readFile(`${base}/field-decisions.jsonl`,'utf8')).trim().split('\n').map(JSON.parse);
const finalSha='06131da4fba897645f485a623af52ae79d080d0e9cfafafc5ccfe891fbd4e0e3';
if(createHash('sha256').update(await readFile('docs/audits/question-verification/remediation/de-it-zh/a2-food-tradition-fixes.mjs')).digest('hex')!==finalSha)throw Error('Unreviewed source');
...
if(fields.length!==8||runtime.length!==42)throw Error('Incomplete followup');
...
console.log(JSON.stringify({all_rows_compared:18,approved_arrays:8,repaired_valid_answers:42}));
