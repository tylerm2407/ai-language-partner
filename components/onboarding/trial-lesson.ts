...
/** Exported for the grading guard in trial-lesson.test.ts. Since onboarding v2
 *  every supported language has a topic pack, so this table is reached only by
 *  a language that has none — but its keys must still grade. */
export const TRIAL_LESSONS: Partial<Record<LanguageCode, Exercise[]>> = {
  es: [
    ex(0, {
