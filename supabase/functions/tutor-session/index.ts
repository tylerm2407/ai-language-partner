...
import { connectTutorProvider } from "../_shared/tutor-provider-call.ts";
...
Deno.serve(async (req: Request): Promise<Response> => {
  ...
  if (
    ...
    return connectTutorProvider(req, supabase, OPENAI_KEY);
  }
  ...
  try {
    if (action === "start") {
      ...
      if (!parsed.ok) {
        return json(400, { error: parsed.error, code: parsed.code });
      }
    ...
    if (action === "turn") {
      ...
      const result = await handleTurn(supabase, userId, parsed.value, {
