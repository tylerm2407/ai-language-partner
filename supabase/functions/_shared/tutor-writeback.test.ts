...
function fakeClient(opts: FakeOptions = {}) {
  ...
  const client = {
    from(table: string) {
      ...
      builder.insert = (payload: Record<string, unknown>) => {
        ...
        if (table === "correction_log") {
          ...
          result =
            (opts.correctionInsertFailsOn ?? []).includes(correctionInserts)
      ...
      builder.update = (payload: Record<string, unknown>) => {
        ...
        if ("analyzed_at" in payload) {
          ...
          if (opts.claimError) {
            ...
            result = {
              data: opts.alreadyClaimed ? [] : [{ id: "session-1" }],
              error: null,
            };
          }
        } else {
    ...
    rpc(name: string, params: Record<string, unknown>) {
      ...
      if (name === "prune_tutor_memory") {
        result = { data: opts.prunedRows ?? 0, error: null };
      }
      if (name === "upsert_tutor_memory") {
        result = { data: "note-id", error: null };
      }
      return {
        then: (onFulfilled: (r: Result) => unknown) =>
          Promise.resolve(result).then(onFulfilled),
      };
    },
  };
...
Deno.test("client-reported transcript writes cards and debrief only", async () => {
  ...
  const ordered = fake.calls.filter((c) =>
    c === "correction_log:insert" || c === "conversation_evidence:insert" ||
    c === "cards:lookup"
  );
  assertEquals(ordered, ["cards:lookup"]);
  ...
  // not contributed to.
  const tail = fake.calls.filter((c) =>
    c.startsWith("rpc:") || c === "tutor_sessions:debrief"
  );
  assertEquals(tail, ["rpc:consume_daily_quota", "tutor_sessions:debrief"]);
});
...
Deno.test("a clean client-reported turn is not measured evidence", async () => {
  const fake = fakeClient();
...
Deno.test("client-reported corrections do not enter correction history", async () => {
  const fake = fakeClient();
  await writeBackTutorSession(fake.client, input());
  assertEquals(fake.rowsIn("correction_log").length, 0);
});
...
Deno.test("an unusable memory note is dropped and the others still land", async () => {
  ...
  assertEquals(result.memoryPruned, 0);
});
...
Deno.test("prune runs once for a session with several notes, not once per note", async () => {
  ...
  assertEquals(
    fake.rpcCalls.filter((c) => c.name === "prune_tutor_memory").length,
    0,
  );
});
...
Deno.test("a failed insert does not abort the rest, and the counts say so", async () => {
  const fake = fakeClient({
    correctionInsertFailsOn: [1],
    evidenceInsertError: "rls denied",
  });
...
Deno.test("a word the learner already studies costs no card and no slot", async () => {
  ...
  assertEquals(
    fake.rpcCalls.filter((c) => c.name === "consume_daily_quota").length,
    0,
  );
});
...
Deno.test("cards are tagged as the tutor surface", async () => {
  ...
  assertEquals(card.tags, ["tutor", "vocabulary", "client_reported"]);
...
Deno.test("an empty analysis writes no learning record at all", async () => {
  ...
  const result = await writeBackTutorSession(
    fake.client,
    input({
      analysis: analysis({ turns: [], vocabulary: [], memoryNotes: [] }),
    }),
  );
