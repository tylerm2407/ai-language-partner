// How long is this audio? Measured from the bytes, estimated from the text.
//
// Pure, no I/O, `deno test`-able.
//
// Lives in _shared/ because two callers need it for different reasons.
// `news-audio` wants a duration to lay out a scrubber. `tts` wants one to
// meter voice minutes: it used to charge a flat 1.0 minute per synthesis
// while `transcribe` charged real audio seconds, so one "voice minute" was
// one unit of paid audio in either direction rather than a minute of
// anything. Both callers prefer the measurement and fall back to the
// estimate — see `estimateDurationMs` at the foot of this file.
//
