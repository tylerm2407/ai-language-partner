-- Fix subscription tier constraint to include 'starter' tier used by app code.
-- The webhook (stripe-webhook) and plan-limits.ts use 'starter' as the free/lowest
-- tier, but the DB constraint only allows ('free', 'basic', 'premium', 'vip').
-- When the webhook writes 'starter', the constraint rejects it.

-- 1. Update the subscriptions table constraint
...
-- 2. Migrate existing 'free' tier rows to 'starter' for consistency
UPDATE public.subscriptions
  SET tier = 'starter'
  WHERE tier = 'free';

-- 3. Update get_effective_limits to handle 'starter' alongside 'free'.
-- The ELSE branch already covers the free/starter case, but we add an explicit
-- 'starter' WHEN clause for clarity and future-proofing. Also handles any
-- lingering 'free' rows that might exist in the subscriptions table.
...
BEGIN
  ...
  SELECT COALESCE(s.tier, 'starter') INTO personal_tier
  FROM public.subscriptions s
  WHERE s.user_id = p_user_id AND s.is_active = TRUE
  LIMIT 1;

  IF personal_tier IS NULL THEN personal_tier := 'starter'; END IF;

  -- Normalize legacy 'free' to 'starter'
  IF personal_tier = 'free' THEN personal_tier := 'starter'; END IF;

  -- Map tier to limits (includes offlineMode)
