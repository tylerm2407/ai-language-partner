-- 140_zh_yuyue_is_an_appointment.sql
--
-- Closes what the round-2 Travel ruling left open: 预约 is an appointment, not a
-- hotel reservation. A2 already glossed it as "Appointment" on eleven rows; B1
-- Travel glossed it as "Reservation" on four, and the SRS card underneath them
-- carried the same error, so every review handed it back.
--
--   snapshot  sha256 b758c2a2ae0797fbdd97402c94ad2a0a872130f3a143c89c6a647dfb6b2b5718
--   rows      6 (1 card, 5 exercises), 10 fields
--   audio     untouched — no patched row stores prompt audio, and the two choice
--             rows change only their English options, never the Chinese played
--
-- Reglossed rather than rekeyed, so the curriculum's noun/verb contrast survives
-- (预约 a noun throughout, 预订 the verb "To book") and no new audio is needed.
-- One exception: "Translate to Chinese: Reservation" in Hotel Check-in is rekeyed
-- to 预订, because a room really is booked with 预订.
--
-- No apply precondition: this depends on no unshipped code.
-- Rollback: docs/audits/question-verification/round4/reverse.sql (NOT applied).
