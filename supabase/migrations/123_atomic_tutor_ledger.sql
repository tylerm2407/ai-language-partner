-- 123 — Make tutor reservation and settlement single database transactions.
--
...
--
-- Two rules folded in from the audit's server-managed-call design (its own
-- call table and RPCs were not adopted; the call model stays migration 113's
-- `call_id` / `connected_at` with the `connect` action):
--   * one OPEN session per learner, serialised by a per-user advisory lock,
--     so two concurrent starts cannot both reserve;
--   * a refund may be SMALLER than the unused share. `refundAllowed` in
--     _shared/tutor-calls.ts forfeits the refund when the provider call could
--     not be confirmed ended, and that forfeit must still settle the row.
...
BEGIN
  ...
  -- Serialise this learner's reservations so two concurrent starts cannot
  -- both pass the open-session check below.
  ...
  END IF;
  -- One open call per learner. The row for THIS start already exists (it is
  -- inserted before reservation so the ledger has something to lock), so it
  -- is excluded; any other open row means a call is still live or unsettled.
  IF EXISTS (
