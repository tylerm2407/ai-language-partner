-- 145 — One writer for the name note.
--
-- Migration 142 seeded the name here; 144 moved that job to a trigger on
-- `user_profiles.display_name`, because onboarding v2 collects the name after
-- this function has already run. This removes the copy left behind. The goal
-- seed stays: `ideal_l2_self` IS in the draft, and this is the statement that
-- stores it.
...
BEGIN
  ...
  -- ── Seed the goal note (142), best-effort (143) ────────────────────────
  --
  -- After the profile, never before. Inside its own block, so a failure here
  -- rolls back the note and nothing else: the row is derived and disposable,
  -- and the profile it was derived from is not.
  --
  -- The NAME is not seeded here any more (migration 145). It is written by the
  -- `fluenci_sync_name_memory` trigger, which fires wherever `display_name` is
  -- actually set — including this statement. Onboarding v2 collects the name
  -- AFTER this flush runs, so the copy that used to live here saw an empty
  -- string and wrote nothing; keeping a second writer for a fact one writer
  -- already owns would only give the two somewhere to disagree.
  BEGIN
