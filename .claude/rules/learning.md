...
- **It is a staircase, not a single-level quiz.** Each strand is asked at the band below, at the band, and at the band above (`STRAND_BAND_OFFSETS`), and the result is read off *where the learner stops passing*. Listening and reading get three rungs (machine-graded, free); writing and speaking get two (a model call / a recording each). Eleven items where there were four.
