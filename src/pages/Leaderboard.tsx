...
import { getLeagueInfo, type League } from '@/lib/gamification'
...
export default function Leaderboard() {
  ...
  useEffect(() => {
    ...
    async function fetchLeaderboard() {
      // Try materialized view first (faster for large user bases)
      if (tab === 'xp') {
        const { data: mvData } = await supabase
          .from('leaderboard_xp')
          .select('id, full_name, username, total_xp, streak_days, avatar_url')
          .order('rank_xp', { ascending: true })
          .limit(50)

        if (mvData && mvData.length > 0) {
          setEntries(mvData as LeaderboardEntry[])
          setLoading(false)
          return
        }
      }
      ...
      const { data } = await supabase
        .from('profiles')
        .select('id, full_name, username, total_xp, streak_days, avatar_url, league')
        ...
        {!loading && entries.length >= 3 && (
          ...
          >
            {[1, 0, 2].map(idx => {
              ...
              const medals = ['🥇', '🥈', '🥉']
              const leagueInfo = e.league ? getLeagueInfo(e.league as League) : null
              return (
                <div key={idx} className="flex flex-col items-center gap-1.5 sm:gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    {leagueInfo && <span className="text-xs">{leagueInfo.icon}</span>}
