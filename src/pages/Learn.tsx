      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 py-4 sm:py-6 space-y-5 sm:space-y-6">
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex items-center justify-between gap-3">
          <div className="min-w-0">
            <h1 className="text-xl sm:text-2xl font-bold flex items-center gap-2">
              <BookOpen className="w-5 h-5 sm:w-6 sm:h-6 text-primary flex-shrink-0" /> Learn
            </h1>
            <p className="text-muted-foreground text-xs sm:text-sm mt-1">Continue where you left off or start a new language.</p>
          </div>
          <Link to="/languages">
            <Button size="sm" variant="outline" className="gap-1 text-xs border-border hover:border-primary/40 flex-shrink-0 min-h-[44px]">
        ...
        {!loading && langProgress.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }}
            className="rounded-2xl border border-dashed border-border p-8 sm:p-10 text-center space-y-3"
          >
            ...
            <Link to="/languages">
              <Button className="bg-gradient-to-r from-primary to-accent text-primary-foreground gap-2 min-h-[48px]">
        ...
        {!loading && langProgress.map((lp, i) => {
          ...
          return (
            <motion.div
              ...
              className="rounded-2xl border border-border bg-card p-4 sm:p-5 hover:border-primary/20 transition-all"
            >
              <div className="flex items-center justify-between gap-3">
                <div className="flex items-center gap-3 sm:gap-4 min-w-0">
                  <div className="text-3xl flex-shrink-0">{lang.flag}</div>
                  <div className="min-w-0">
                    ...
                    <div className="flex items-center gap-2 sm:gap-3 text-xs text-muted-foreground mt-1 flex-wrap">
                      ...
                      <span className="flex items-center gap-1"><BookOpen className="w-3 h-3" /> {lp.words_read_count.toLocaleString()}</span>
                      <span className="flex items-center gap-1 hidden sm:flex"><MessageSquare className="w-3 h-3" /> {lp.sessions_completed}</span>
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3" /> {timeAgo(lp.last_activity_at)}</span>
                    </div>
                  </div>
                </div>
                <Link to={`/learn/${lang.slug}`}>
                  <Button size="sm" className="gap-1 bg-gradient-to-r from-primary to-accent text-primary-foreground text-xs flex-shrink-0 min-h-[44px]">
